...
class GuitarTabParserTest < ActiveSupport::TestCase
  ...
  test "it parses sections with plain old chords" do
    expected = [{ section: { header: { header_name: "Intro", header_fluff: [] }, contents: { song_lines: { chord_line: [{ chord: { chord_root: "E", major_minor: "m" } }, { chord: { chord_root: "Bb", major_minor: nil } }, { chord: { chord_root: "C", major_minor: "m" } }] } } } }, { section: { header: { header_name: "Verse", header_fluff: [] }, contents: { song_lines: { chord_line: [{ chord: { chord_root: "E", major_minor: "m" } }, { chord: { chord_root: "A", major_minor: nil } }, { chord: { chord_root: "C", major_minor: "m" } }] } } } }]
  ...
  test "it recognizes tab sections with chords and lyrics interspersed" do
    ...
    assert_equal parsed[0][:section][:header][:header_name], "Verse"
    assert parsed[0][:section][:contents][:tab_lines]

    assert_nil parsed[1][:section][:header]
    assert parsed[1][:section][:contents][0][:lyric_line]

    assert_nil parsed[2][:section][:header]
    assert parsed[2][:section][:contents][:tab_lines]
